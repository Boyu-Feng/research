...
try:
    # optional decay transformer (drop-in alternative)
    from llama.new.model_with_decay import TransformerWithDecay
except Exception:
    TransformerWithDecay = None
...
class Llama:
    ...
    def build(
        ...
        if use_decay_model and TransformerWithDecay is not None:
            model = TransformerWithDecay(model_args).to(device)
        else:
    ...
    def __init__(self, model: Transformer, tokenizer: Tokenizer, device: str):
        ...
        # detect if the loaded model supports `key_decay` in forward
        self._supports_key_decay = hasattr(self.model.forward, "__call__") and (
            "key_decay" in self.model.forward.__code__.co_varnames
            if hasattr(self.model.forward, "__code__")
            else False
        )
    ...
    def generate(
        ...
        for cur_pos in range(min_prompt_len, total_len):
            # compute key decay tensor when requested and supported by model
            logits = None
            if decay_alpha > 0 and self._supports_key_decay:
                # Use generation-step-based decay: a single scalar per generation
                # step is applied to all key positions. This makes the prompt's
                # attention uniformly decay as we generate more tokens.
                # gen_step counts how many tokens have been generated so far
                # (1 for the first generated token).
                gen_step = float(cur_pos - min_prompt_len + 1)
                kd_scalar = float(torch.exp(-decay_alpha * gen_step))
                if decay_min > 0.0:
                    kd_scalar = max(kd_scalar, decay_min)
                # keys cover positions [0, cur_pos-1]
                cache_len = cur_pos
                # broadcast scalar to (bsz, cache_len)
                kd = torch.full((bsz, cache_len), kd_scalar, device=self.device, dtype=torch.float32)
                try:
                    logits = self.model.forward(tokens[:, prev_pos:cur_pos], prev_pos, key_decay=kd)
                except TypeError:
                    # model doesn't accept key_decay despite inspection
